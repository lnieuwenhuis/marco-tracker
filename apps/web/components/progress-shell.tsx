...
function GoalsPanel({
  ...
  const [weightKg, setWeightKg] = useState(
    initialWeightKg != null ? formatInputValue(initialWeightKg) : "",
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <section
        aria-labelledby="macro-calculator-title"
        className="space-y-4 rounded-[1.45rem] border border-[var(--color-border)] bg-[var(--color-shell-panel)] p-4 sm:p-5"
      >
        <div>
          <h3 id="macro-calculator-title" className="text-sm font-bold text-[var(--color-ink)]">
            Macro calculator
          </h3>
        ...
        <div className="grid grid-cols-2 gap-3" aria-label="Sex">
          <ChoiceButton
            label="Male"
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <NumberField label="Age" unit="yrs" step="1" value={age} disabled={isPending} onChange={updateCalculator(setAge)} />
          <NumberField label="Height" unit="cm" step="0.1" value={heightCm} disabled={isPending} onChange={updateCalculator(setHeightCm)} />
          <NumberField label="Weight" unit="kg" step="0.1" value={weightKg} disabled={isPending} onChange={updateCalculator(setWeightKg)} />
        </div>
          ...
          {calculatedTargets ? (
            <div className="space-y-4">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <ResultTile label="Target" value={`${calculatedTargets.targetCaloriesKcal} kcal`} />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <ResultTile label="Protein" value={`${formatInputValue(calculatedTargets.macros.proteinG)} g`} />
                <ResultTile label="Carbs" value={`${formatInputValue(calculatedTargets.macros.carbsG)} g`} />
                <ResultTile label="Fat" value={`${formatInputValue(calculatedTargets.macros.fatG)} g`} />
                <ResultTile label="Calories" value={`${calculatedTargets.macros.caloriesKcal} kcal`} />
              </div>
